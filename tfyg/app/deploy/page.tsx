  ...
  return (
    <div className="container mx-auto py-8">
      <div className="flex items-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-center flex-1 pr-8">Deploy Arbitrage Contract</h1>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="bg-zinc-900 border-zinc-800">
          <CardHeader className="border-b border-zinc-800">
            <CardTitle>Deploy ArbitrageExecutor Contract</CardTitle>
            <CardDescription>Deploy the contract to a testnet or mainnet</CardDescription>
          </CardHeader>
          <CardContent className="pt-6">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-zinc-500">Get an RPC URL from Alchemy, Infura, or another provider</p>
              </div>
              ...
              {deployedAddress && (
                <Alert className="bg-green-900/50 border-green-800">
                  ...
                  <AlertDescription>
                    Contract Address: <code className="bg-black/30 p-1 rounded">{deployedAddress}</code>
                  </AlertDescription>
                </Alert>
              )}
            </div>
          ...
          <CardFooter className="border-t border-zinc-800 bg-zinc-900/50">
            <Button
              ...
              disabled={isDeploying || !rpcUrl || !privateKey}
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          <CardHeader className="border-b border-zinc-800">
            ...
            <CardDescription>Real-time deployment progress</CardDescription>
          </CardHeader>
          <CardContent className="p-0">
            <div className="bg-black/50 p-4 h-[400px] overflow-y-auto font-mono text-xs">
              {deploymentLog.length > 0 ? (
                deploymentLog.map((log, index) => (
                  <div key={index} className="py-1 border-b border-zinc-900 last:border-0">
          ...
          <CardFooter className="border-t border-zinc-800 bg-zinc-900/50">
            <div className="w-full">
              <div className="flex justify-between items-center mb-2">
                <span className="text-xs text-zinc-500">Deployment Progress</span>
                <span className="text-xs text-zinc-500">
                  {deploymentStep > 0 ? `${Math.min(deploymentStep, 9)}/9` : "Not started"}
                </span>
              </div>
              <div className="w-full bg-zinc-800 rounded-full h-2.5">
                <div
                  className="bg-blue-600 h-2.5 rounded-full"
                  style={{ width: `${(deploymentStep / 9) * 100}%` }}
                ></div>
              </div>
            </div>
          </CardFooter>
